		...
		>
			...
			${activeCompare ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
				...
				>
					<Zap
						size={16}
						className={`sm:w-[18px] sm:h-[18px] transition-all duration-300 ${activeCompare ? `${getItemTypeColor(type, true)} scale-110` : 'text-white/50 hover:text-white'}`}
			...
			${isAdded ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
				...
				>
					<Heart
						size={16}
						className={`sm:w-[18px] sm:h-[18px] transition-all duration-300 ${isAdded ? `${getItemTypeColor(type, true)} scale-110` : 'text-white/50 hover:text-white'}`}
			...
			>
				...
				<div className="p-2 sm:p-3 flex flex-col gap-1.5 sm:gap-2">
					<h3
						className={`text-white font-bold text-xs sm:text-sm line-clamp-2 sm:line-clamp-1 ${getHoverColor(type)} transition-colors leading-tight`}
					>
						{title}
					</h3>
					...
					<div className="flex items-center justify-between mt-auto gap-2 text-[9px] sm:text-xs">
						...
						<span
							className={`text-[8px] sm:text-[10px] uppercase tracking-tighter font-extrabold ${getItemTypeColor(type)} bg-opacity-10 px-1.5 sm:px-2 py-0.5 rounded-md border ${getItemTypeColor(type)}/20 flex-shrink-0`}
						>
