...
import { OpenLibraryBookDetails, OpenLibraryCoversResult, OpenLibraryGenre, OpenLibrarySearch } from './Openlibrary.type'
...
export const openLibraryApi = createApi({
	...
	endpoints: builder => ({
		searchBooks: builder.query<OpenLibrarySearch, { query: string; page?: number }>({
			query: ({ query, page = 1 }) => ({
				url: 'search.json',
				params: {
					q: query,
		...
		getAuthorDetails: builder.query<{ name: string; photos?: number[]; bio?: string }, string>({
			query: (authorKey) => `${authorKey}.json`,
		}),
		getBooksByGenre: builder.query<OpenLibraryGenre, { genre: string; page?: number; limit?: number }>({
		...
		getBookCovers: builder.query<number[], string>({
			...
			transformResponse: (response: OpenLibraryCoversResult) => {
				const allCovers = response.entries
					.flatMap((entry: OpenLibraryCoversResult['entries'][number]) => entry.covers || [])
